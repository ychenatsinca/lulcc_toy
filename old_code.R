...
#colnames(df) <- c('x', 'y', 'topo')
#define topology from df 
#topo_raster <- rasterFromXYZ(df)  #Convert first two columns as lon-lat and third as value                

# Assign land-use type to a 2D array lu_arr
# 3:forest; 2:agriculture; 1:built-up 
lu_arr <- array(3, dim=c(nx,ny)) 

# assign the land use type to city
#lu_arr[1,5] <- 1
#lu_arr[8,1] <- 1
# assign the inital state of LULCC by surrounding the city with agricultural lands  
#agriculture around
#for city[1,5]
#lu_arr[1,4]   <- 2; lu_arr[1,6]   <- 2; lu_arr[2,4:6] <- 2
#for city[8,1] 
#lu_arr[9,1]   <- 2; lu_arr[7,1]   <- 2; lu_arr[7:9,2] <- 2
