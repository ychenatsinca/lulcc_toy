...
####################################################################################################
fun_lulcc_toy <- function(ld_plot=TRUE, c_value=400, f_demo=0.00005, agb_ratio = 0.25,
                          topo_file="dtm_taipei_100_100.tiff",f_top=0.06, nx=100,ny=100,
                          geo_city = data.frame(lon_x=c(78, 80, 50, 20, 10),
                                                lat_y=c(48, 35, 75, 65, 40)),
                          crop_para=data.frame(y=c(100,120), p=c(10,10), 
...
# plot the results    
  ...
  if (ld_plot) {
    ...
    contour(topo, add=TRUE)
    #add text (pixels of the city) on the plot 
    for (k in 1: n_city) {
        text(x =geo_city$lon_x[k]/100, y=geo_city$lat_y[k]/100, labels=round(pix_city[k]),col="black", cex=2.0)
    }
  }#end of ld_plot 
  
  
  print("The Total Pixel of the City(ies) :")
  print( round(pix_city) )
  
  return( round(pix_city) )
