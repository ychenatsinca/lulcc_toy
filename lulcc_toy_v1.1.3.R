...
# revised 2022-03-20 add soil PH data/map into the consideration of crop-yields
# Authors: Yi-Ying Chen and Meng-Hsuan Vivian Lin   
# Contact email: yiyingchen@gate.sinica.edu.tw 
#             or as0210250@gate.sinica.edu.tw
####################################################################################################
#fun_lulcc_toy <- function(ld_plot=TRUE, c_value=300, f_demo=0.00005, agb_ratio = 0.25,
#                          topo_file="dtm_taipei_100_100.tif",f_top=0.05, nx=100,ny=100,
#                          ph_file="soil_ph_taipei_100_100.tif",f_ph=c(0.1,0.5),
#                          geo_city = data.frame(lon_x=c(78*1, 80*1, 50*1, 20*1, 10*1),
#                                               lat_y=c(48*1, 35*1, 75*1, 65*1, 40*1)),
#                          crop_para=data.frame(y=c(90,105), p=c(10,10), 
...
# load R-package "raster" 
  ...
  if (ld_go) {
    ...
    ph_file="soil_ph_taipei_100_100.tif"
  ...
  if (ld_topo) {
    ...
    #import the real topographic data from Geo-tiff image (100 by 100)
    #topo_map <- raster("dtm_taipei_100_100.tif")
    ph_map <- raster(x=ph_file)
    ph <- t(as.matrix(ph_map))[,ny:1]
    ph[is.na(ph)] <- 0
  ...
  for (k in 1:n_city) {
    ...
    for (i in 1:nx) {
          ...
          tmp <- 1.0
          #apply the suitablity of soil Ph for agricuture
          if (( 4.5 <= ph[i,j]) & (ph[i,j] <= 7.5) ) {
            tmp <- 1.0
          } else {
            tmp <- abs((1.0 - abs(7.0 - ph[i,j])*0.10))
          }
          
           rent_n[i,j,k,l] <- (crop_para$p[l]*crop_para$y[l]) * tmp*f_ph[l]  -
